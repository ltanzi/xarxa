...
export async function PostCard({ post }: { post: PostWithAuthor }) {
  ...
  return (
    <div className="group relative py-6 border-b border-fg/10 transition-opacity hover:opacity-60">
      ...
      <h3 className={`relative text-lg font-light break-words ${post.closed ? "text-muted" : ""}`}>{post.title}</h3>
      <p className="relative mt-2 text-sm text-muted line-clamp-2 leading-relaxed max-w-2xl">{post.description}</p>
      <div className="relative mt-3 flex items-center gap-3 text-xs text-muted">
        <Link
          href={`/profile/${post.author.id}`}
          className="font-medium text-fg hover:underline underline-offset-4 z-10"
        >
          {authorName}
        </Link>
        {post.location && (
